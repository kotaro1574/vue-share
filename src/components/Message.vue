    ...
    <div v-for="(value, index) in shares" :key="index">
      <div class="message">
        <div class="flex">
          ...
          <img src="../assets/heart.png" alt="" class="icon" @click="fav(index)" />
...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://frozen-falls-83149.herokuapp.com/api/like",
              data: {
        ...
        axios
          .post("https://frozen-falls-83149.herokuapp.com/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    del(index) {
      axios
        .delete(
          "https://frozen-falls-83149.herokuapp.com/api/shares/" +
